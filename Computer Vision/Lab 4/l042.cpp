...
void setupPPM(string filename)
{
    ofstream ppmout(filename + ".ppm");
...
void part1()
{
    ...
    draw_circle(3, ch[-1], "256000000");
    setupPPM("quickhull");
    

}
Point p0;
double polar_angle(Point x, Point y)
{
    Line temp = Line(x, y);
    double rad = atan(temp.getSlope()) * 180.0 / (M_PI);
    if(rad > 0)
        return rad;
    else if(rad < 0)
        return 180.0 + rad;
...
double orientation(Point x, Point y, Point z) //orientation of ordered X, Y, X, return 1 if clockwise, only one I need to worry about
{ 
    double val = (y.y_coord - x.y_coord) * (z.x_coord - y.x_coord) 
                    - (y.x_coord - x.x_coord) * (z.y_coord - y.y_coord);
    if(val > 0) return 1;  //clockwise
    else if(val < 0) return -1; //counterclockwise
    return 0; //colinear
}
bool cmp3(Point a, Point b)
{
    // double temp = orientation(p0, a, b);
    // if(temp == -1)
    //     return true;
    // else if(temp == 1)
    //     return false;
    // double one = find_distance(p0, a); double two = find_distance(p0, b);
    // if(one < two)
    //     return true;
    // return false;
    // if(polar_angle(p0, a) != polar_angle(p0, b))
    // {return polar_angle(p0, a) < polar_angle(p0, b);}
    // return find_distance(p0, a) < find_distance(p0, b);
    ...
    {double o = -(a.getX() - p0.getX()) / (a.getY() - p0.getY());
    double t = -(b.getX() - p0.getX()) / (b.getY() - p0.getY());
    if(o - t < 0)
    {
        return true;
    }
...
void part2()
{
    vector<Point> pts = input_vector();
    // double min = pts[0].y_coord; int min_index = 0;
    // for(int i = 1; i < (int)(pts.size()); i++) //this is wrong right now
    // {
    //     double y_min = pts[i].y_coord;
    //     if((y_min < min) || (y_min == min) && (pts[i].x_coord < pts[min].x_coord))
    //         min = y_min; min_index = i;
    // }
    // Point temp = pts[min_index];
    // pts[min_index] = pts[0];
    // pts[0] = temp;
    sort(pts.begin(), pts.end(), cmp2);
    p0 = pts[0];
    sort(pts.begin() + 1, pts.end(), cmp3);
    int m = 1;
    for(int i = 1; i < (int)(pts.size()); i++)
    {
        while(i < (int)(pts.size()) - 1 && orientation(p0, pts[i], pts[i+1]) == 0)
            i++;
        pts[m] = pts[i];
        m++;
    }
    ...
    // draw_circle(3, pts[1], "256000000");
    // draw_circle(3, pts[2], "256000000");
    for(int i = 2; i < m; i++)
    {
        while(s.size() > 1)
        {
            Point t = s.top();
            Point tt = nextToTop(s);
            if(orientation(tt, t, pts[i]) == 1)
                s.pop();
            else if(orientation(tt, t, pts[i]) == -1)
                break;
        }
        s.push(pts[i]);
    }
    ...
    {
        Line temp = Line(output[i], output[i+1]);
        temp.drawLine("256000000");
    }
    Line final = Line(output[(int)output.size()-1], output[0]);
    final.drawLine("256000000");
    setupPPM("grahamscan");
}
int main()
{
