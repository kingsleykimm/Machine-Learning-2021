...
{
    unsigned long long int x = p.getX()/d*2;
    unsigned long long int y = p.getY()/d*2;
...
//         cout << i << endl;
        vector<Point> pts;
        for(int k= (p.first - 2); k <= (int)(p.first + 2); k++) //25 closest squares lookup for points
        {
            for(int j = (p.second-2); j <= (int)(p.second+2); j++)
            {
                pair<unsigned long long int, unsigned long long int> p = make_pair<unsigned long long int, unsigned long long int>(k ,j);
                if(dict.count(p))
                {
        ...
        //vector is intialized with points, now need to look through it to find distances smaller
    ...
    //                         dp.print(); cout << i << endl;
            ...
            {
                dict[detSubSquare(pts[a], d)] = pts[a];
            }
